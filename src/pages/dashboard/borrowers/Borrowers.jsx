...
const DATA = [
  { id: "b1", name: "Alice Johnson",  email: "alice@acme.com",    score: 720, tier: "Prime",      status: "Active" },
  { id: "b2", name: "Carlos Mendes",  email: "carlos@beta.io",    score: 645, tier: "Near-prime", status: "Under Review" },
  { id: "b3", name: "Priya Sharma",   email: "priya@delta.co",    score: 580, tier: "Subprime",   status: "Active" },
  { id: "b4", name: "James Wright",   email: "james@ewcorp.net",  score: 490, tier: "Deep sub",   status: "Defaulted" },
  { id: "b5", name: "Dana Lee",       email: "dana@foxtrot.com",  score: 700, tier: "Prime",      status: "Active" },
  { id: "b6", name: "Emeka Obi",      email: "emeka@goldinv.ng",  score: 610, tier: "Near-prime", status: "Active" },
];
  ...
  return (
    <div>
      ...
      <div style={s.tableWrap}>
        <table style={s.table}>
          <thead>
            <tr>{["Name", "Email", "Score", "Tier", "Status", ""].map((h) => <th key={h} style={s.th}>{h}</th>)}</tr>
          </thead>
          <tbody>
            {rows.map((b) => (
              <tr key={b.id} style={s.tr}>
                ...
                <td style={s.td}><span style={{ ...s.badge, ...STATUS_STYLE[b.status] }}>{b.status}</span></td>
                <td style={s.td}><Link to={`/dashboard/borrowers/${b.id}`} style={s.viewLink}>View →</Link></td>
              </tr>
            ))}
          </tbody>
        </table>
...
const s = {
  ...
  viewLink:  { color: "#111", textDecoration: "none", fontSize: "13px", fontWeight: "500" },
};
